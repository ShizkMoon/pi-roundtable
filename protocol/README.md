...
Workspace profile persistence, secure provider/MCP credential references, frozen participant prompts/model routes, catalog import status, explicit Skill/MCP grants, and the encrypted role-memory storage foundation are implemented in the Windows local-client path. The Runtime Host resolves installed catalog entries, executes tools from an approved and attached MCP server with per-tool interactive approval, and runs bounded isolated SubAgents. Automatic memory extraction/recall, the memory review UI, prompt-revision history, live capability-change events, and end-of-meeting promotion review remain planned; lifecycle events do not claim those services are implemented.
