...
- [x] `V04-REL-007` Implement the optional production-UpgradeCode clean-VM
  evidence harness with explicit disposable-VM attestation and exact
  run-owned cleanup. It is a manual diagnostic, not a personal release gate.
...
- [x] `V04-WIN-002` Extract `MeetingCommandGateway` first: public prompt,
  private prompt, discussion controls, approval decisions, receipt mapping,
  cancellation, and active-generation propagation. Status: `implemented`; all
  sends are fenced by the active generation and the boundary has unit coverage.
- [ ] `V04-WIN-003` Extract `MeetingSessionController`: start, pause, resume,
  ...
  Status: `scaffolded`; full-history load, missing/stale checkpoint
  reconciliation, and role-visible recovery context are implemented, while
  runtime start/pause/close orchestration still resides in the presentation
  adapter.
- [x] `V04-WIN-004` Extract `MeetingProjectionController`: accepted event ->
  C++ reducer -> durable append -> UI projection, preserving sequence order.
  Status: `implemented` with fail-closed sequence and persistence behavior.
- [x] `V04-WIN-005` Extract workspace, role-profile, and catalog controllers one
  use case at a time with locked behavior tests before moving code. Status:
  `implemented` for the current use cases; further ViewModel reduction remains
  incremental work under `V04-WIN-007`.
- [x] `V04-WIN-006` Add explicit diagnostics for stale-generation drops,
  duplicate sequence, sequence gap, persistence failure, and projection failure
  without persisting private payloads. Status: `implemented`; diagnostic text is
  closed and omits event payloads.
- [ ] `V04-WIN-007` Keep `MainViewModel` as a presentation adapter; set and
  ...
  rewrite. Status: `scaffolded`; dependency boundaries and a 4,810-line ceiling
  are enforced, but more feature logic still needs extraction.
...
- [x] `V04-GATE-001` C++ configure/build/test, TypeScript clean build/tests,
  Windows Release tests, schema checks, and PowerShell parse gate pass.
- [x] `V04-GATE-002` Existing runtime generation, recovery, memory, document,
  updater, signing-pipeline, MSI-contract, and session-transfer tests show no
  regression.
- [x] `V04-GATE-003` Self-contained publish and Runtime Host/protocol import
  smoke pass from a clean staging directory.
- [x] `V04-GATE-004` Default ICE, administrative extraction, isolated full MSI
  install/launch/repair/upgrade/downgrade block/second repair/uninstall pass for
  the v0.4 candidate.
- [x] `V04-GATE-005` Signed manifest, exact re-download, updater re-verification,
  and rollback/tamper scenarios pass.
- [x] `V04-GATE-006` Create and publish `v0.4.0` only from protected `main` and
...
- [x] `V05-CTX-001` Implement private `ProviderCapabilityProfileV1` for OpenAI,
  ...
  name substring alone. Status: `implemented` and unit-verified; real-provider
  verification remains in `V05-GATE-001` through `V05-GATE-003`.
- [ ] `V05-CTX-002` Separate stable role prefix, session-frozen memory context,
  ...
  state in `RoleContextSnapshotV1`. Status: `scaffolded`; the bounded schema,
  fingerprint, and validator exist, but production session assembly is pending.
- [ ] `V05-CTX-003` Preserve byte-for-byte stable reusable prefixes; record a
  ...
  Status: `scaffolded`; stable prompts and invalidation classification are
  unit-verified, while lifecycle-wide invalidation reporting is pending.
- [x] `V05-CTX-004` Map OpenAI automatic prefix caching, Anthropic explicit
  cache breakpoints/TTL, DeepSeek automatic disk cache, and unsupported-provider
  no-op behavior behind the private adapter. Status: `implemented` and
  unit-verified; provider-specific behavior is not yet real-provider verified.
- [x] `V05-CTX-005` Add bounded `ProviderUsageSampleV1` parsing for provider
  input/output/cache-read/cache-write tokens; missing fields remain unknown,
  never fabricated zeroes. Status: `implemented` and unit-verified through the
  private Runtime Host diagnostics listener.
- [ ] `V05-CTX-006` Surface cache eligibility, hit rate, token savings estimate,
  invalidation cause, and provider support in a local diagnostics UI. Status:
  `pending`; the bounded private diagnostic records exist, but no UI consumes them.
- [ ] `V05-CTX-007` Add synthetic compatible-provider fixtures: ignored cache
  ...
  streaming usage, retry, and provider model-window disagreement. Status:
  `scaffolded`; alias, missing-field, partial-usage, safe no-op, and model-window
  disagreement fixtures exist, while rejected-field and retry fixtures remain.
- [x] `V05-CTX-008` Provide bounded built-in provider web search for direct
  roles and isolated SubAgents. Status: `implemented` for the official OpenAI
  Responses route with grant, model, network-policy, call-count, timeout, and
  size gates; only structured citations cross the tool boundary and private
  queries/results are omitted from events and diagnostics. The provider seam is
  replaceable and unit-verified; real-provider verification remains `pending`.

### 4.2 Compaction pipeline

- [x] `V05-CMP-001` Implement `ContextCompactionRecordV1` with trigger ratio,
  before/after token estimates, retained turn range, summary digest, duration,
  provider/model, and failure/fallback reason. Status: `implemented` and
  unit-verified on Pi compaction start/end events without summary plaintext.
- [x] `V05-CMP-002` Use actual resolved context windows when reliable; fall back
  conservatively when unknown. Reserve output and tool-call headroom.
  Status: `implemented` and unit-verified; long-session verification is pending.
- [ ] `V05-CMP-003` Keep recent turns verbatim, summarize older public context,
  ...
  provider reasoning as meeting facts. Status: `pending`.
- [ ] `V05-CMP-004` Persist resumable role context snapshots at turn boundaries;
  ...
  policy version before restore. Status: `scaffolded`; bounded snapshot creation
  and validation exist, while turn-boundary persistence and restore are pending.
- [ ] `V05-CMP-005` On corruption/incompatibility, discard only the private
  ...
  Status: `pending`; rejection reasons exist, but rebuild orchestration does not.
- [ ] `V05-CMP-006` Add adversarial long-session tests for late constraints,
  ...
  compaction, restart, and stale-generation writes. Status: `pending`.

### 4.3 Long-term role memory

- [x] `V05-MEM-001` Migrate `roundtable.db` to v3 with encrypted
  ...
  tables and rollback atomically on failure. Status: `implemented` with exact
  fresh/v2 migration and schema validation tests.
- [x] `V05-MEM-001A` Require `V04-DAT-001` and the reviewed v3 schema contract
  from `V04-DAT-002` before the first production migration is merged. Status:
  `implemented`; the migration preserves the previously reviewed ownership
  boundary in ADR 0013.
- [ ] `V05-MEM-002` Add memory candidate APIs for manual, review-required, and
  ...
  optimistic decision revision. Status: `scaffolded`; manual proposal,
  source/confidence fields, atomic approve/reject, and optimistic decision
  revision are implemented. Meeting-close generation and automated safety scan
  remain `pending`.
- [ ] `V05-MEM-003` Add Windows memory UI for active entries, candidates,
  ...
  clear recovery diagnostics. Status: `scaffolded`; the complete manual review
  and revision loop is connected, while retention controls and corruption
  recovery UI remain `pending`.
- [x] `V05-MEM-004` Freeze one bounded recall set when a role session starts:
  maximum four entries plus character/token budget, deterministic ranking, and
  explicit untrusted-background delimiters. Status: `implemented` with a 6,000
  character budget and private Runtime Host injection.
- [ ] `V05-MEM-005` Record which memory IDs/revisions were considered/selected
  ...
  Status: `scaffolded`; append-only audits contain only considered/selected
  revision references. A closed reason taxonomy remains `pending`.
- [x] `V05-MEM-006` Ensure active-session recall never changes silently; edits
  affect the next role-session freeze unless an explicit restart command is
  accepted under the active generation. Status: `implemented`; memory mutation
  is disabled while running and the recall set is frozen per role start.
...
- [ ] `V06-ART-001` Implement private `ArtifactDescriptorV1`: artifact ID,
  ...
  Status: `scaffolded`; the composer-owned import descriptor and meeting binding
  subset are implemented without local paths, while producer, sensitivity, and
  general worker fields remain `pending`.
...
- [x] `V06-ART-004` Use content-addressed immutable storage plus a transactional
  index and reference counts. Write through temp + flush + atomic promote;
  reopen and verify every generated file before indexing it. Status:
  `implemented` for imported composer attachments, including source re-hash,
  shared meeting bindings, and last-reference cleanup.
- [ ] `V06-ART-005` Add quota, preview/delete/restore, retention jobs,
  ...
  Status: `scaffolded`; bounded quota, unbound-LRU cleanup, and startup orphan
  reconciliation are implemented. Preview/restore, backup, retention jobs, and
  quarantine remain `pending`.
- [x] `V06-ART-006` Connect composer attachment selection, preflight summary,
  warning/removal, cancellation, retry, and send-failure preservation. Do not
  put local absolute paths or file bytes into normalized events. Status:
  `implemented` for the public composer; private-composer attachment parity is
  `pending`.
...
- [ ] `V08-GATE-005` Publish `v0.8.0` from protected `main` only after required CI,
  hashes, SBOM/licenses, migration/rollback notes, support matrix, known
  limitations, and re-downloaded asset verification all pass. Production
  signing remains optional for the personal project.
...
The active v0.4 candidate ledger is [`docs/release/ledger/v0.4.0.md`](../release/ledger/v0.4.0.md).
...
- [ ] optional real-provider support matrix and timestamp;
- [ ] MSI/bundle/module sizes, file counts, install/repair/upgrade/uninstall time;
- [ ] optional clean-VM and previous-release upgrade evidence;
- [ ] SHA-256, SBOM, licenses, and notices; optional signatures and timestamp authority;
- [ ] optional real scenario script, result, redactions, and known limitations;
