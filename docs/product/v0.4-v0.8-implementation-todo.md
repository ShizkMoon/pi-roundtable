...
- [ ] `V04-REL-001` Add one canonical repository version record plus an
  ...
  manifest inputs.
- [ ] `V04-REL-001A` Before an RC is built, advance every current-build binding
  ...
  sources.
- [ ] `V04-REL-002` Add a version-consistency gate that fails on stale or mixed
  release literals, but allow explicitly labelled historical evidence.
- [ ] `V04-REL-003` Add one local quality entry point with `fast`, `windows`,
  and `release-candidate` scopes. Preserve required CI job names `cpp-core` and
  `typescript`.
- [ ] `V04-REL-004` Parse every PowerShell script in the fast Windows gate;
  introduce pinned PSScriptAnalyzer only after a clean baseline and a measured
  runtime budget are recorded.
- [ ] `V04-REL-005` Add schema catalog checks for valid JSON, unique `$id`,
  local `$ref` resolution, declared draft, and additive v1 evolution fixtures.
- [ ] `V04-REL-006` Remove stale current-release wording from README and
...
- [ ] `V04-DST-003` Define a versioned signed-catalog envelope verifier with
...
- [ ] `V04-DST-004` Define a credential-free verification result and diagnostic
...
- [ ] `V04-DST-006` Make staging handle-relative and reparse-safe: lock parent
  ...
  acceptable release boundary.

### 3.3 Windows application seams

- [ ] `V04-WIN-001` Move concrete store/service construction behind an
...
- [ ] `V04-RUN-001` Extract `RuntimeGenerationOwner` and lease lifecycle from
  the monolithic local host; validate initialize/start/stop/late-command races.
- [ ] `V04-RUN-002` Extract `MeetingCommandRouter` with deterministic command
  serialization, idempotent receipt behavior, and fail-closed generation and
  sequence checks.
- [ ] `V04-RUN-003` Extract `RoleSessionSupervisor` and keep every Pi session,
  credential lease, abort controller, MCP client, and SubAgent scoped to one
  role and one runtime generation.
- [ ] `V04-RUN-004` Introduce `RoleContextAssembler`,
  `ProviderCapabilityRegistry`, and `CapabilityResolver` interfaces while
  preserving current Pi behavior.
- [ ] `V04-RUN-005` Introduce `DiscussionOrchestrator` and
...
- [ ] `V04-RUN-006` Replace ambient credential lifetime with an explicit
...
- [ ] `V04-DAT-001` Add direct schema tests for fresh initialization, v1 -> v2
...
- [ ] `V04-DAT-002` Specify, but do not create empty production tables for,
  `roundtable.db` v3 memory candidates/recall audits/context snapshots.
- [ ] `V04-DAT-003` Specify separate `platform.db` ownership, schema version,
