...
Normalized domain events are the authoritative meeting history. Provider transcripts, reasoning records, caches, tool arguments/results, and Pi private sessions are private runtime state. Windows durable checkpoints, crash-safe command receipts, human approval before granted MCP tools, and MCP consent boundaries are implemented. The relay's authenticated PostgreSQL storage boundary is implemented and locally verified, while Windows/Android authenticated recovery over that relay remains pending. OpenTelemetry GenAI spans, content-safe diagnostics export, and broader budget/loop limits remain planned integration surfaces; their schemas must remain additive and must not weaken `runtimeGeneration` fencing.
