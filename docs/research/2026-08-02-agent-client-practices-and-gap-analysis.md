...
- Windows 长记录虚拟化和自适应断点有自动化/静态覆盖。真实交互桌面已在 150% DPI 下完成 720/900/1280/1520 DIP、Markdown、设置页和键盘焦点的像素验收；托管桌面若连 `PrintWindow` 与屏幕捕获都不可用，脚本仍会把 `visualStatus` 标为 `pending`，只保存明确标注为非视觉等价物的 UIA 结构快照。100%/200%、深色和系统高对比仍 pending。
...
3. 720/900/1280/1520 DIP 断点、UIA 名称/焦点和浅色结构有自动覆盖，并已在 150% DPI 的真实交互桌面完成像素核验；100%/200%、深色和实际系统高对比仍需补充人工视觉矩阵。
