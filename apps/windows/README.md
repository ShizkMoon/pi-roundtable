...
The first installer target is an x64/ARM64 MSI that includes the .NET runtime, Windows App SDK, Node Runtime Host, and native meeting core. The project already selects self-contained framework/runtime deployment. Provider/model persistence, provider model-list discovery/import, and API-key storage in Windows Credential Manager are implemented. Git Skill/MCP imports use a bounded temporary checkout, a redacted LLM review snapshot, an isolated atomic catalog install, and an explicit disabled-until-approved lifecycle. Signing, upgrade/repair behavior, bundled Node, ARM64 native-core packaging, and MSI creation remain planned.
